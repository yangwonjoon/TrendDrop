const trends = [
  {
    ...
    summary: "운동 루틴과 다이어트 브이로그에서 반복 노출되며 저장과 공유가 빠르게 늘어나는 중입니다.",
  ...
  },
  {
    ...
    summary: "주말 소비를 겨냥한 여행 추천 포스트가 검색과 댓글 반응을 동시에 끌고 있습니다.",
  ...
  },
  {
    ...
    summary: "퍼스널 컬러 기반 짧은 튜토리얼이 저장 수와 재방문을 끌어올리고 있습니다.",
  ...
  },
  {
    ...
    summary: "재택근무와 공부 브이로그에서 반복 등장하며 구매 링크 클릭이 이어지고 있습니다.",
  ...
  },
  {
    ...
    summary: "초보자 러닝 인증 콘텐츠가 릴레이처럼 이어지며 태그 노출량이 늘어나는 흐름입니다.",
  ...
  },
  {
    ...
    summary: "업무 자동화 관련 추천 게시물이 세이브와 북마크를 빠르게 만들고 있습니다.",
...
const categories = ["전체", ...new Set(trends.map((trend) => trend.category))];
...
function renderTrends() {
  const visibleTrends = activeCategory === "전체"
    ? trends
    : trends.filter((trend) => trend.category === activeCategory);

  trendGrid.innerHTML = visibleTrends
    .map(
      (trend) => `
        <article class="trend-card">
          <div class="trend-head">
            ...
            <span class="trend-tag">${trend.category}</span>
          </div>
          <p class="trend-meta">${trend.summary}</p>
          <div class="trend-stats">
            <div class="stat-block">
              <div class="stat-value">${trend.growth}</div>
              <p class="stat-label">검색 상승률</p>
            </div>
            <div class="stat-block">
              <div class="stat-value">${trend.velocity}</div>
              <p class="stat-label">확산 속도</p>
            </div>
          </div>
          <p class="trend-meta"><strong>왜 뜨나:</strong> ${trend.reason}</p>
          <p class="trend-source">감지 채널: ${trend.source}</p>
        </article>
      `
    )
    .join("");
...
function renderTimeline() {
  timelineList.innerHTML = timelineSteps
    .map(
      (item) => `
...
function renderWatchlist() {
  watchlist.innerHTML = watchItems
    .map(
      (item) => `
        <li>
          <div class="watch-keyword">
            <strong>${item.keyword}</strong>
            <p class="watch-meta">${item.meta}</p>
          </div>
          <span class="watch-score">${item.score}</span>
        </li>
      `
    )
    .join("");
